...
//	function mi nt ( address , uint) external ;
}
interface ISales {
	struct Mint_data {
...
//		uint256 admin_fee_amount = remaining_amount * admin_fee_rate / 10000 ;
        ...
        uint256 tokenid ;
		if ( ( tokenid = IKIP17( _target_kip17_contract )._itemhash_tokenid( _itemid ) ) == 0 ) {
			tokenid = IKIP17( _target_kip17_contract ).mint (
			_seller // _sell er
			, _itemid
...
/*						emit PaidFee (							address(this)							, author_royalty_amount							, uint256(SEND_TYPES.AUTHOR_ROYALTY_DEPOSIT) 							, _author						);
*/
					}
            ...
            if ( _referer_fee_rate > 0){
                if ( _referer == address(0) ){}
                else {
                    uint256 _referer_fee_amount = _amounttopay * _referer_fee_rate / 10000 ;
                    IERC20 ( _paymeansaddress).transfer ( _referer , _referer_fee_rate ) ;
                }
            } else {}			 		
